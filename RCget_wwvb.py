...
PWM date-time code from radio station WWVB
...
"the time code is always transmitted in the minute immediately after the moment it represents"
...
specification is BitA 200 - 500 ms, BitB 500 - 800 ms
bitA is the timecode data, bitB is just for end of timecode detection
'''
show_smpls = False  # use to check for clean pulses
A1, A2 = 21, 48  # bit A, 21,48 = 210-480 ms
B1, B2 = 52, 78  # bit B, 52,78 = 520-780 ms
...
if not abort:
    ...
    if gmtime()[0] > 2021:
        ...
        # WWVB transmits UTC for the current minute
        year, _, _, hour, minute, _, _, doy = gmtime(time()) # type: ignore
    ...
    while True:
        ...
        if ticks_ms()%1000 == delta_t:
            ...
            if tcB.endswith('0011'):
                ...
                if not match[0] and tcA[-16] != ' ':
                    yr = int(tcA[-16:-12],2)*10 + int(tcA[-11:-7],2)  # decode BCD to decimal
                ...
                if tcA[-60]!=' ':
                    ...
                    if all(match) and ((len(mods)>1 and len(set(mods)) == 1) or mods.count(mod) >= 3):
                        ...
                        while month_doys[month] < doy:
                            ...
                            month +=1
                        # need to add one minute   # WWVB transmits current minute not next minute
                        utc = gmtime(mktime((year, month, day, hour, minute+1, 0, 0,0,0)))
                        ...
                        print(f'Set to WWVB {gmtime()[2]} {months[gmtime()[1]]} {gmtime()[0]}  {gmtime()[3]:02.0f}:{gmtime()[4]:02.0f} (UTC)')
                        break
